...
describe('lib/mediaObjects', () => {
	testInFiber('buildFormatString', () => {
		const format1 = buildFormatString(literal<MediaInfo>({
			name: '',
		...
		const format2 = buildFormatString(literal<MediaInfo>({
			name: '',
		...
		const format3 = buildFormatString(literal<MediaInfo>({
			name: '',
	...
	testInFiber('acceptFormat', () => {
		const accepted1 = acceptFormat('1920x1080i2500tff', [
			['1920', '1080', 'i', '2500', 'tff']
		])
		expect(accepted1).toEqual(true)

		const accepted2 = acceptFormat('1920x1080i2500tff', [
			['1280', '720', 'p', '2500']
		])
