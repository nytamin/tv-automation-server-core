...
export function matchFilter(
	...
	if (filter) {
		...
		if (filter.outputLayerIds !== undefined && filter.outputLayerIds.indexOf(item.outputLayerId) < 0) {
			return false
		}
		// Source layers
		if (filter.sourceLayerIds !== undefined && filter.sourceLayerIds.indexOf(item.sourceLayerId) < 0) {
			return false
		}
		// Source layer types
...
export function fetchAndFilter(props: Translated<IAdLibPanelProps>): IAdLibPanelTrackedProps {
	...
	AdLibPieces.find(
		{
			rundownId: {
				$in: props.playlist.getRundownIDs(),
			},
			partId: {
