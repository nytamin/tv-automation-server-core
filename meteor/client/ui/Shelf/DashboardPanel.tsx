...
> {
	...
	isAdLibOnAir(adLib: AdLibPieceUi) {
		if (
			this.props.unfinishedPieceInstanceIds[unprotectString(adLib._id)] &&
			this.props.unfinishedPieceInstanceIds[unprotectString(adLib._id)].length > 0
		) {
			return true
		}
		return false
	...
	render() {
		if (this.props.visible && this.props.showStyleBase && this.props.filter) {
			...
			if (!this.props.uiSegments || !this.props.playlist) {
				...
				return (
					<div className="dashboard-panel" style={dashboardElementPosition(filter)}>
...
export function getUnfinishedPieceInstancesReactive(currentPartInstanceId: PartInstanceId | null) {
	...
	_.each(
		...
		(grp, id) => (unfinishedPieceInstanceIds[id] = _.map(grp, (instance) => instance._id))
	)
