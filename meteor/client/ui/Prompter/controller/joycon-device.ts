...
export class JoyconController extends ControllerAbstract {
	...
	private static readonly JOYCON_SLACK = 0.25
	...
	private _updateScrollPosition() {
		...
		if (pad) {
			...
			if (stickPos >= JoyconController.JOYCON_SLACK) {
				// filters out deadband
				const mappedInput =
					0 + (1 - 0) * ((stickPos - JoyconController.JOYCON_SLACK) / (1 - JoyconController.JOYCON_SLACK))
		...
		if (this._currentPosition !== undefined && scrollPosition !== undefined) {
			if (this._currentPosition === scrollPosition) {
				...
				if (!this._hasReachedEnd && this._lastSpeed !== 0) {
					...
					this._haptic.playEffect('dual-rumble', {
						duration: 0.03,
						startDelay: 0,
